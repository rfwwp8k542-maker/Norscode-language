    ...
    static int nl_json_skip_array_raw(const char *s, int *idx, int len) {
        ...
        static nl_map_text *nl_web_dispatch_params(nl_map_text *routes, const char *method, const char *path) {
            ...
            int start() {
                ...
                nl_map_text* params = nl_web_path_params("/brukere/{id}/poster/{slug}", "/brukere/7/poster/introduksjon");
                nl_map_text* routes = nl_map_text_new();
                nl_map_text_set(routes, "hjem", "GET /");
                nl_map_text_set(routes, "bruker", "GET /brukere/{id}");
                nl_map_text_set(routes, "poster", "POST /brukere/{id}/poster/{slug}");
                nl_print_text("Bruker-id:");
                nl_print_text(nl_map_text_get_required(params, "id"));
                nl_print_text("Slug:");
                nl_print_text(nl_map_text_get_required(params, "slug"));
                nl_print_text("Rute:");
                nl_print_text(nl_web_dispatch(routes, "GET", "/brukere/7"));
                if (nl_web_path_match("/brukere/{id:int}", "/brukere/7")) {
                    nl_print_text("Gyldig numerisk path-param");
                }
                nl_call_stack_len = nl_call_stack_mark;
                return 0;
                nl_call_stack_len = nl_call_stack_mark;
