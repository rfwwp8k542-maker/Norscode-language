...
- binær-first dokumentasjon ble gjort til standard
- legacy Python-vei ble tydelig merket i CLI og bootstrap
...
- `run` ble native-first
- `check` ble standardisert som semantikk + native validering
- `build` ble bytecode-first
- `test` ble native-first
...
- `LockCheck` ble gjort mer legacy-lik ved å ignorere `generated_at`
...
- toppnivå-CLI fikk native bridge for `lock`
- `lock`
- `lock --check`
- `lock --verify`
  bruker nå native bro som standard
- `lock --legacy-python` ble lagt inn som eksplisitt fallback

## `update`
- første Rust-preview ble opprettet
- native bridge via toppnivå-CLI ble lagt inn
- dependency-plan med `unchanged`/`skipped` ble lagt inn i preview
...
- første smale native skrivevei ble lagt inn for:
  - direkte upinnet `git+...`
  - direkte `url+...`
- enkel `update` og enkel `update --check` bruker nå native bro som standard
- `update --legacy-python` ble lagt inn som eksplisitt fallback

## `add`
- første Rust-preview ble opprettet
- native bridge via toppnivå-CLI ble lagt inn
- preview fikk klassifisering av eksisterende dependencies:
  - `path`
  - `git`
  - `url`
- preview fikk klassifisering av ønsket input:
  - `name`
  - `path`
  - `git`
  - `url`
- preview fikk `target_status`, `target_existing_kind` og `target_plan`
- første smale native mutasjonsvei ble lagt inn for:
  - direkte `path`
  - `add <navn> <path>`
  - direkte `git+...`
  - direkte `url+...`
- docs og CLI-hjelp ble gjort ærlige om at `--git` og `--url` fortsatt er legacy-spor

## `registry-sync`
- første Rust-preview ble opprettet
- native bridge via toppnivå-CLI ble lagt inn
- preview begynte å lese:
  - standard registry-sti
  - om registry finnes
  - `package_count`
- `status` og `target` ble lagt inn som baseline i output
- første smale native write-steg ble lagt inn:
  - lokal init av `packages/registry.toml` med minimalt grunnformat
- enkel `registry-sync` uten avanserte source/policy-flagg bruker nå native bro som standard
- `registry-sync --legacy-python` ble lagt inn som eksplisitt fallback

## `registry-sign`
- første Rust-preview ble opprettet
- native bridge via toppnivå-CLI ble lagt inn
- preview eksponerer nå:
  - config
  - legacy-status
  - registry-sti
  - om registry finnes
...
- dokumentasjon ble flyttet fra Python-first til binær-first
- prosjektkommandoer ble tydelig merket i CLI og README
- legacy-veier ble gjort eksplisitte i stedet for skjulte standarder

## Åpne migrasjonsområder
- `add` er fortsatt ikke fullverdig native standardkommando
- `registry-sign` mangler første smale write-steg og senere `--write-config`
- `registry-mirror` mangler native preview/flyt
- `ci` mangler native inngang i brukerflaten
