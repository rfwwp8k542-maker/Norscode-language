...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    int start() {
        nl_assert_eq_int(nl_text_length("æøå"), 3);
        nl_assert(nl_text_er_ordtegn("Å"));
        nl_assert_eq_text(nl_text_slice("Hei æøå", 4, 7), "æøå");
        nl_assert_eq_text(nl_text_slice("", 0, 0), "");
        nl_assert_eq_text(nl_text_trim("   "), "");
        nl_assert_eq_text(nl_text_erstatt("æøå", "ø", "o"), "æoå");
        nl_assert_eq_int(nl_list_text_len(nl_split_words("hei  æøå")), 2);
        nl_list_text* tokens = nl_tokenize_simple("navn_1 # kommentar\nx-2");
        nl_assert_eq_int(nl_list_text_len(tokens), 2);
        nl_assert_eq_text(tokens->data[0], "navn_1");
        nl_assert_eq_text(tokens->data[1], "x-2");
        nl_assert(nl_text_inneholder("Håkon", "åk"));
        return 0;
        return 0;
    }
