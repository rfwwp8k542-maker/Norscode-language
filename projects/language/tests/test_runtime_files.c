...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    int start() {
        char * rot = "build/runtime_files_probe";
        char * filsti = nl_concat(rot, "/hello.no");
        char * kilde = "funksjon start() -> heltall { returner 0 }";
        nl_assert_eq_int(nl_write_file(filsti, kilde), 1);
        nl_assert(nl_file_exists(filsti));
        nl_assert_eq_text(nl_read_file(filsti), kilde);
        nl_assert((nl_list_text_len(nl_list_files(rot)) > 0));
        nl_assert((nl_list_text_len(nl_list_files_tree("build")) > 1));
        return 0;
        return 0;
    }
