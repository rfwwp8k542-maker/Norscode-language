...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    int start() {
        ...
        int _tittel_id = std_gui__tekst(panel_id, "Norscode Todo");
        int input_id = std_gui__tekstboks(panel_id, "");
        int knapp_id = std_gui__knapp(panel_id, "Legg til");
        int _liste_id = std_gui__liste(panel_id);
        int status_id = std_gui__tekst(panel_id, "Klar");
        std_gui__p__klikk(knapp_id, "legg_til_punkt");
        std_gui__trykk(knapp_id);
        nl_assert_eq_text(std_gui__hent_tekst(status_id), "Skriv noe først");
        std_gui__sett_tekst(input_id, "Kjøp melk");
        std_gui__trykk(knapp_id);
        nl_assert_eq_text(std_gui__hent_tekst(input_id), "");
        nl_assert_eq_text(std_gui__hent_tekst(status_id), "Punkt lagt til");
