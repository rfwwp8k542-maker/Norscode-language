...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    nl_list_text* tom_tekst() {
        return nl_list_text_new();
        return nl_list_text_new();
    }
    
    int sjekk(nl_list_text* l) {
        nl_assert_eq_int(nl_list_text_len(l), 0);
        return 0;
        return 0;
    }
    
    int start() {
        nl_assert_eq_int(sjekk(nl_list_text_new()), 0);
        nl_assert_eq_int(sjekk(tom_tekst()), 0);
        return 0;
        return 0;
    }
