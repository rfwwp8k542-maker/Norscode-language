...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    int start() {
        nl_assert_eq_text(nl_int_to_text((-(12))), "-12");
        nl_assert_eq_text(nl_bool_to_text(0), "usann");
        nl_assert_eq_int(nl_text_to_int("  -12 "), (-(12)));
        nl_assert_eq_int(nl_text_to_int("ikke et tall"), 0);
        nl_assert_eq_int(nl_text_length(nl_int_to_text(0)), 1);
        return 0;
        return 0;
    }
