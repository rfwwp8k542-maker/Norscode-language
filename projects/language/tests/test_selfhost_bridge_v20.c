...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    int start() {
        ...
        for (int __nl_i_1 = 0; __nl_i_1 < nl_list_int_len(tall); __nl_i_1++) {
            int x = tall->data[__nl_i_1];
            sum = (sum + x);
        }
        int bonus = (((sum == 6)) ? 4 : 0);
        int resultat = (((bonus == 4)) ? (sum + bonus) : 0);
        nl_assert_eq_int(sum, 6);
        nl_assert_eq_int(bonus, 4);
        nl_assert_eq_int(resultat, 10);
        return 0;
        return 0;
    }
