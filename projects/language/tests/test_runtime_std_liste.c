...
static int nl_run_command(nl_list_text *parts) {
    ...
    for (int i = 0; i < parts->len; i++) {
        char *part = parts->data[i] ? nl_strdup(parts->data[i]) : nl_strdup("");
        char *next = i == 0 ? nl_strdup(part) : nl_concat(nl_concat(cmd, " "), part);
        free(cmd);
        free(part);
        cmd = next;
    }
...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    static char *nl_sass_replace_variables(const char *text, nl_list_text *names, nl_list_text *values) {
        ...
        for (int i = 0; i < nl_list_text_len(names); i++) {
            char *next = nl_text_erstatt(current, names->data[i] ? names->data[i] : "", values->data[i] ? values->data[i] : "");
            free(current);
            current = next;
        }
        return current;
    ...
    int start() {
        ...
        nl_assert_eq_int(std_liste__fjern_tall(tall, 1), 2);
        ...
        nl_assert_eq_int(std_liste__fjern_tekst(tekster, 0), 2);
        return 0;
        return 0;
    }
