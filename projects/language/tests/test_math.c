...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    int start() {
        ...
        nl_assert_eq_int((10 - 4), 6);
        nl_assert_eq_int((3 * 4), 12);
        nl_assert_eq_int((8 / 2), 4);
        int s = std_math__pluss(10, 20);
        nl_assert_eq_int(s, 30);
        int m = std_math__minus(50, 8);
        nl_assert_eq_int(m, 42);
        nl_assert((5 > 3));
        nl_assert((3 < 5));
        nl_assert_eq_int((5 >= 5), 1);
        nl_assert_eq_int((4 <= 5), 1);
        nl_print_text("test_math OK");
        return 0;
        return 0;
    }
