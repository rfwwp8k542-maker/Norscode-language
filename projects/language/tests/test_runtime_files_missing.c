...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    int start() {
        char * manglende_fil = "build/runtime_missing/nope.no";
        char * manglende_mappe = "build/runtime_missing/nope_dir";
        nl_assert_ne_int(nl_file_exists(manglende_fil), 1);
        nl_assert_eq_int(nl_list_text_len(nl_list_files(manglende_mappe)), 0);
        nl_assert_eq_int(nl_list_text_len(nl_list_files_tree(manglende_mappe)), 0);
        return 0;
        return 0;
    }
