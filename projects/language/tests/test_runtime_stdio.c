...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    char * std_io__prompt(char * spm) {
        return nl_read_input(spm);
        return "";
    }
    
    char * std_io__prompt_med_prefiks(char * prefiks, char * spm) {
        return nl_concat(prefiks, nl_read_input(spm));
        return "";
    }
    
    char * std_io__skriv_linje(char * verdi) {
        nl_print_text(verdi);
        return verdi;
        return "";
    }
    
    int std_io__skriv_tall(int verdi) {
        nl_print_text(nl_int_to_text(verdi));
        return verdi;
        return 0;
    }
    
    int std_io__skriv_boolsk(int verdi) {
        nl_print_text(nl_bool_to_text(verdi));
        return verdi;
        return 0;
    }
    
    int start() {
        nl_assert_eq_text(std_io__prompt("Skriv noe"), "");
        nl_assert_eq_text(std_io__skriv_linje("hei"), "hei");
        nl_assert_eq_int(std_io__skriv_tall(7), 7);
        nl_assert_eq_int(std_io__skriv_boolsk(1), 1);
        return 0;
        return 0;
    }
