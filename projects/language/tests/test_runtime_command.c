...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    int start() {
        nl_list_text* kommando = nl_list_text_new();
        nl_list_text_push(kommando, "python3");
        nl_list_text_push(kommando, "-c");
        nl_list_text_push(kommando, "import sys; sys.exit(0)");
        nl_assert_eq_int(nl_run_command(kommando), 0);
        nl_assert_eq_int(nl_run_command(nl_list_text_new()), 1);
        return 0;
        return 0;
    }
