...
static nl_list_text *nl_split_lines(const char *s) {
    ...
    int start() {
        nl_assert(nl_file_exists("tests/test_runtime_builtins.no"));
        char * path_verdi = nl_read_env("PATH");
        nl_assert((nl_text_length(path_verdi) > 0));
        char * input_verdi = nl_read_input("Skriv noe");
        nl_assert_eq_text(input_verdi, "");
        nl_assert_eq_int(nl_text_length("Hei verden"), 10);
        nl_assert_eq_text(nl_int_to_text(42), "42");
        nl_assert_eq_text(nl_bool_to_text(1), "sann");
        nl_assert_eq_int(nl_text_to_int(" 17 "), 17);
        nl_assert_eq_int(nl_text_slutter_med("Hei verden", "verden"), 1);
        nl_assert_eq_text(nl_text_trim("  hei  "), "hei");
        ...
        nl_assert_eq_int(nl_list_text_len(nl_split_words("hei   verden")), 2);
        nl_list_text* enkle_tokens = nl_tokenize_simple("abc # kommentar\ndef-g");
        nl_assert_eq_int(nl_list_text_len(enkle_tokens), 2);
        nl_assert_eq_text(enkle_tokens->data[0], "abc");
        nl_assert_eq_text(enkle_tokens->data[1], "def-g");
        nl_list_text* uttrykk_tokens = nl_tokenize_expression("la x = 10 + y");
        nl_assert_eq_int(nl_list_text_len(uttrykk_tokens), 6);
        nl_assert_eq_text(uttrykk_tokens->data[0], "la");
        nl_assert_eq_text(uttrykk_tokens->data[1], "x");
        nl_assert_eq_text(uttrykk_tokens->data[2], "=");
        nl_assert_eq_text(uttrykk_tokens->data[3], "10");
        nl_assert_eq_text(uttrykk_tokens->data[4], "+");
        nl_assert_eq_text(uttrykk_tokens->data[5], "y");
        nl_assert_eq_text(nl_sass_to_css("$farge: red;\n.knapp {\n  color: $farge;\n}"), ".knapp { color: red; }\n");
        nl_assert(nl_text_er_ordtegn("a"));
        nl_assert_ne_int(nl_text_er_ordtegn("-"), 1);
