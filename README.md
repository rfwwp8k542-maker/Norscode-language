...
- Utvide selv-hosting til full parser (påbegynt: uttrykksparser + mini-skriptparser med `la`/`sett`/`returner`, compound assignment (`+=`, `-=`, `*=`, `/=`, `%=`), nestede `hvis ... da ... ellers ...` (uttrykk + assignment), tomme statements (`;`) og norske operator-ord (`og/eller/ikke`, `er/ikke_er`, `mindre_enn`/`storre_enn`) inkludert fraser med mellomrom (`mindre enn`, `ikke er`, `er ikke`, `er lik`, `er ikke lik`) i `selfhost/compiler.no`)
