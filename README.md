...
- Utvide selv-hosting til full parser (påbegynt: uttrykksparser + mini-skriptparser med `la`/`sett`/`returner`, compound assignment (`+=`, `-=`, `*=`, `/=`, `%=`), nestede `hvis ... da ... ellers ...` (uttrykk + assignment), tomme statements (`;`) og norske operator-ord (`og/eller/ikke`, `er/ikke_er`, `mindre_enn`/`storre_enn`) inkludert fraser med mellomrom (`mindre enn`, `ikke er`, `er ikke`, `lik`, `er lik`, `er lik med`, `ikke lik`, `ikke lik med`, `ulik`, `ulik med`, `er ulik`, `er ikke lik`, `er ikke lik med`, `er mindre enn`, `er storre enn`, `er mindre eller lik`, `er storre eller lik`, `mindre enn eller lik`, `storre enn eller lik`, `mindre enn lik`, `storre enn lik`, `mindre lik`, `storre lik`, `er mindre lik`, `er storre lik`) i `selfhost/compiler.no`)
