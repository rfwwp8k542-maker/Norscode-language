...
- Utvide selv-hosting til full parser (påbegynt: uttrykksparser + mini-skriptparser med `la`/`sett`/`returner`, nestede `hvis ... da ... ellers ...` (også i assignment), tomme statements (`;`) og norske bool-operator-ord (`og/eller/ikke`) i `selfhost/compiler.no`)
