use crate::project::ProjectRoot;
use sha2::{Digest, Sha256};
use std::fs;
use std::path::{Path, PathBuf};

#[derive(Debug, Clone)]
pub struct RegistryProjectRoot {
    pub project: ProjectRoot,
}

impl RegistryProjectRoot {
    pub fn discover_from_cwd() -> Option<Self> {
        ProjectRoot::discover_from_cwd().map(|project| Self { project })
    }
}

#[derive(Debug, Clone)]
pub struct RegistrySyncPreview {
    pub config_path: String,
    pub uses_legacy_config: bool,
    pub status: String,
    pub target: String,
    pub default_registry_path: String,
    pub registry_exists: bool,
    pub package_count: usize,
    pub mode: String,
}

#[derive(Debug, Clone)]
pub struct RegistrySyncWriteResult {
    pub config_path: String,
    pub status: String,
    pub target: String,
    pub registry_path: String,
    pub changed: bool,
    pub package_count: usize,
    pub mode: String,
}

#[derive(Debug, Clone)]
pub struct RegistrySignPreview {
    pub config_path: String,
    pub uses_legacy_config: bool,
    pub status: String,
    pub registry_path: String,
    pub registry_exists: bool,
    pub registry_sha256: Option<String>,
    pub digest_path: String,
    pub mode: String,
}

#[derive(Debug, Clone)]
pub struct RegistrySignWriteResult {
    pub config_path: String,
    pub uses_legacy_config: bool,
    pub status: String,
    pub registry_path: String,
    pub registry_exists: bool,
    pub registry_sha256: Option<String>,
    pub digest_path: String,
    pub changed: bool,
    pub mode: String,
}

#[derive(Debug, Clone)]
pub struct RegistryMirrorPreview {
    pub config_path: String,
    pub uses_legacy_config: bool,
    pub status: String,
    pub target: String,
    pub default_output_path: String,
    pub registry_exists: bool,
    pub package_count: usize,
    pub mode: String,
}

#[derive(Debug, Clone)]
pub struct RegistryMirrorWriteResult {
    pub config_path: String,
    pub uses_legacy_config: bool,
    pub status: String,
    pub target: String,
    pub output_path: String,
    pub registry_exists: bool,
    pub package_count: usize,
    pub changed: bool,
    pub mode: String,
}

pub fn preview_registry_sync(source_override: Option<&str>) -> Option<RegistrySyncPreview> {
    let project_root = RegistryProjectRoot::discover_from_cwd()?;
    let config_path = project_root.project.config.display().to_string();
    let registry_path = project_root
        .project
        .root
        .join("packages")
        .join("registry.toml");
    if source_override.is_some_and(is_registry_sync_remote_source) {
        return Some(RegistrySyncPreview {
            config_path,
            uses_legacy_config: project_root.project.uses_legacy_config(),
            status: "error".to_string(),
            target: "explicit-source".to_string(),
            default_registry_path: registry_path.display().to_string(),
            registry_exists: false,
            package_count: 0,
            mode: "source-remote-unsupported".to_string(),
        });
    }
    let source_path = resolve_registry_sync_source_path(&project_root.project.root, source_override);
    let effective_path = source_path.as_ref().unwrap_or(&registry_path);
    let registry_exists = effective_path.exists();
    let package_count = read_registry_package_count(effective_path);
    let default_registry_path = registry_path.display().to_string();

    Some(RegistrySyncPreview {
        config_path,
        uses_legacy_config: project_root.project.uses_legacy_config(),
        status: "preview".to_string(),
        target: if source_override.is_some() {
            "explicit-source".to_string()
        } else {
            "default".to_string()
        },
        default_registry_path,
        registry_exists,
        package_count,
        mode: if source_override.is_some() {
            "preview-explicit-source".to_string()
        } else {
            "preview".to_string()
        },
    })
}

pub fn try_registry_sync_local(source_override: Option<&str>) -> Option<RegistrySyncWriteResult> {
    let project_root = RegistryProjectRoot::discover_from_cwd()?;
    let config_path = project_root.project.config.display().to_string();
    let registry_path = project_root.project.root.join("packages").join("registry.toml");
    let parent = registry_path.parent()?;
    let existed = registry_path.exists();
    if source_override.is_some_and(is_registry_sync_remote_source) {
        return Some(RegistrySyncWriteResult {
            config_path,
            status: "error".to_string(),
            target: "explicit-source".to_string(),
            registry_path: registry_path.display().to_string(),
            changed: false,
            package_count: 0,
            mode: "source-remote-unsupported".to_string(),
        });
    }
    let source_path = resolve_registry_sync_source_path(&project_root.project.root, source_override);

    if !parent.exists() {
        fs::create_dir_all(parent).ok()?;
    }

    let changed = if let Some(source_path) = source_path {
        if !source_path.exists() {
            return Some(RegistrySyncWriteResult {
                config_path,
                status: "error".to_string(),
                target: "explicit-source".to_string(),
                registry_path: registry_path.display().to_string(),
                changed: false,
                package_count: 0,
                mode: "source-missing".to_string(),
            });
        }
        let source_text = fs::read_to_string(&source_path).ok()?;
        match fs::read_to_string(&registry_path) {
            Ok(existing) if existing == source_text => false,
            _ => {
                fs::write(&registry_path, source_text).ok()?;
                true
            }
        }
    } else if !existed {
        fs::write(&registry_path, "[packages]\n").ok()?;
        true
    } else {
        false
    }

    Some(RegistrySyncWriteResult {
        config_path,
        status: if changed {
            "updated".to_string()
        } else if source_override.is_some() || existed {
            "unchanged".to_string()
        } else {
            "updated".to_string()
        },
        target: if source_override.is_some() {
            "explicit-source".to_string()
        } else {
            "default".to_string()
        },
        registry_path: registry_path.display().to_string(),
        changed,
        package_count: read_registry_package_count(&registry_path),
        mode: if source_override.is_some() {
            if changed {
                "source-copy".to_string()
            } else {
                "source-noop".to_string()
            }
        } else if existed {
            "local-noop".to_string()
        } else {
            "local-init".to_string()
        },
    })
}

pub fn preview_registry_sign() -> Option<RegistrySignPreview> {
    let project_root = RegistryProjectRoot::discover_from_cwd()?;
    let config_path = project_root.project.config.display().to_string();
    let registry_path = project_root.project.root.join("packages").join("registry.toml");
    let digest_path = project_root
        .project
        .root
        .join("packages")
        .join("registry.toml.sha256");
    let registry_exists = registry_path.exists();
    let registry_sha256 = compute_registry_sha256(&registry_path);

    Some(RegistrySignPreview {
        config_path,
        uses_legacy_config: project_root.project.uses_legacy_config(),
        status: "preview".to_string(),
        registry_path: registry_path.display().to_string(),
        registry_exists,
        registry_sha256,
        digest_path: digest_path.display().to_string(),
        mode: "preview".to_string(),
    })
}

pub fn try_registry_sign_write_digest() -> Option<RegistrySignWriteResult> {
    let project_root = RegistryProjectRoot::discover_from_cwd()?;
    let config_path = project_root.project.config.display().to_string();
    let registry_path = project_root.project.root.join("packages").join("registry.toml");
    let digest_path = project_root
        .project
        .root
        .join("packages")
        .join("registry.toml.sha256");
    let registry_exists = registry_path.exists();
    let registry_sha256 = compute_registry_sha256(&registry_path);

    if !registry_exists || registry_sha256.is_none() {
        return Some(RegistrySignWriteResult {
            config_path,
            uses_legacy_config: project_root.project.uses_legacy_config(),
            status: "error".to_string(),
            registry_path: registry_path.display().to_string(),
            registry_exists,
            registry_sha256,
            digest_path: digest_path.display().to_string(),
            changed: false,
            mode: "sidecar-missing-registry".to_string(),
        });
    }

    let digest = registry_sha256.clone().unwrap_or_default();
    let changed = match fs::read_to_string(&digest_path) {
        Ok(existing) if existing.trim() == digest => false,
        _ => {
            fs::write(&digest_path, format!("{digest}\n")).ok()?;
            true
        }
    };

    Some(RegistrySignWriteResult {
        config_path,
        uses_legacy_config: project_root.project.uses_legacy_config(),
        status: if changed {
            "updated".to_string()
        } else {
            "unchanged".to_string()
        },
        registry_path: registry_path.display().to_string(),
        registry_exists,
        registry_sha256,
        digest_path: digest_path.display().to_string(),
        changed,
        mode: if changed {
            "sidecar-write".to_string()
        } else {
            "sidecar-noop".to_string()
        },
    })
}

pub fn try_registry_sign_write_config() -> Option<RegistrySignWriteResult> {
    let project_root = RegistryProjectRoot::discover_from_cwd()?;
    let config_path = project_root.project.config.clone();
    let config_path_display = config_path.display().to_string();
    let registry_path = project_root.project.root.join("packages").join("registry.toml");
    let digest_path = project_root
        .project
        .root
        .join("packages")
        .join("registry.toml.sha256");
    let registry_exists = registry_path.exists();
    let registry_sha256 = compute_registry_sha256(&registry_path);

    if !registry_exists || registry_sha256.is_none() {
        return Some(RegistrySignWriteResult {
            config_path: config_path_display,
            uses_legacy_config: project_root.project.uses_legacy_config(),
            status: "error".to_string(),
            registry_path: registry_path.display().to_string(),
            registry_exists,
            registry_sha256,
            digest_path: digest_path.display().to_string(),
            changed: false,
            mode: "config-write-missing-registry".to_string(),
        });
    }

    let digest = registry_sha256.clone().unwrap_or_default();
    let changed = upsert_trusted_registry_sha256(&config_path, &digest).ok()?;

    Some(RegistrySignWriteResult {
        config_path: config_path_display,
        uses_legacy_config: project_root.project.uses_legacy_config(),
        status: if changed {
            "updated".to_string()
        } else {
            "unchanged".to_string()
        },
        registry_path: registry_path.display().to_string(),
        registry_exists,
        registry_sha256,
        digest_path: digest_path.display().to_string(),
        changed,
        mode: if changed {
            "config-write".to_string()
        } else {
            "config-noop".to_string()
        },
    })
}

pub fn preview_registry_mirror(output_override: Option<&str>) -> Option<RegistryMirrorPreview> {
    let project_root = RegistryProjectRoot::discover_from_cwd()?;
    let config_path = project_root.project.config.display().to_string();
    let registry_path = project_root.project.root.join("packages").join("registry.toml");
    let default_output_path = resolve_registry_mirror_output_path(&project_root.project.root, output_override);
    let registry_exists = registry_path.exists();
    let package_count = read_registry_package_count(&registry_path);

    Some(RegistryMirrorPreview {
        config_path,
        uses_legacy_config: project_root.project.uses_legacy_config(),
        status: "preview".to_string(),
        target: if output_override.is_some() {
            "explicit-output".to_string()
        } else {
            "default".to_string()
        },
        default_output_path: default_output_path.display().to_string(),
        registry_exists,
        package_count,
        mode: if output_override.is_some() {
            "preview-explicit-output".to_string()
        } else {
            "preview".to_string()
        },
    })
}

pub fn try_registry_mirror_write_default(output_override: Option<&str>) -> Option<RegistryMirrorWriteResult> {
    let project_root = RegistryProjectRoot::discover_from_cwd()?;
    let config_path = project_root.project.config.display().to_string();
    let registry_path = project_root.project.root.join("packages").join("registry.toml");
    let output_path = resolve_registry_mirror_output_path(&project_root.project.root, output_override);
    let registry_exists = registry_path.exists();
    let package_count = read_registry_package_count(&registry_path);

    if !registry_exists {
        return Some(RegistryMirrorWriteResult {
            config_path,
            uses_legacy_config: project_root.project.uses_legacy_config(),
            status: "error".to_string(),
            target: "default".to_string(),
            output_path: output_path.display().to_string(),
            registry_exists,
            package_count,
            changed: false,
            mode: "default-write-missing-registry".to_string(),
        });
    }

    let parent = output_path.parent()?;
    if !parent.exists() {
        fs::create_dir_all(parent).ok()?;
    }

    let payload = format!(
        "{{\n  \"source\": \"packages/registry.toml\",\n  \"package_count\": {package_count}\n}}\n"
    );
    let changed = match fs::read_to_string(&output_path) {
        Ok(existing) if existing == payload => false,
        _ => {
            fs::write(&output_path, payload).ok()?;
            true
        }
    };

    Some(RegistryMirrorWriteResult {
        config_path,
        uses_legacy_config: project_root.project.uses_legacy_config(),
        status: if changed {
            "updated".to_string()
        } else {
            "unchanged".to_string()
        },
        target: if output_override.is_some() {
            "explicit-output".to_string()
        } else {
            "default".to_string()
        },
        output_path: output_path.display().to_string(),
        registry_exists,
        package_count,
        changed,
        mode: if changed {
            if output_override.is_some() {
                "explicit-write".to_string()
            } else {
                "default-write".to_string()
            }
        } else {
            if output_override.is_some() {
                "explicit-noop".to_string()
            } else {
                "default-noop".to_string()
            }
        },
    })
}

pub fn future_registry_note() -> &'static str {
    "Registry-logikk skal bo i egen prosjektmodul, ikke i runtime-kjernen."
}

fn read_registry_package_count(registry_path: &std::path::Path) -> usize {
    let Ok(raw) = std::fs::read_to_string(registry_path) else {
        return 0;
    };

    let mut current_section = String::new();
    let mut count = 0;
    for line in raw.lines() {
        let trimmed = line.trim();
        if trimmed.is_empty() || trimmed.starts_with('#') {
            continue;
        }
        if trimmed.starts_with('[') && trimmed.ends_with(']') {
            current_section = trimmed[1..trimmed.len() - 1].trim().to_string();
            continue;
        }
        if (current_section == "packages" || current_section == "registry.packages")
            && trimmed.contains('=')
        {
            count += 1;
        }
    }
    count
}

fn compute_registry_sha256(registry_path: &std::path::Path) -> Option<String> {
    let bytes = fs::read(registry_path).ok()?;
    let mut hasher = Sha256::new();
    hasher.update(bytes);
    Some(format!("{:x}", hasher.finalize()))
}

fn resolve_registry_sync_source_path(project_root: &Path, source_override: Option<&str>) -> Option<PathBuf> {
    let raw = source_override?;
    let candidate = PathBuf::from(raw);
    Some(if candidate.is_absolute() {
        candidate
    } else {
        project_root.join(candidate)
    })
}

fn is_registry_sync_remote_source(raw: &str) -> bool {
    raw.contains("://")
}

fn resolve_registry_mirror_output_path(project_root: &Path, output_override: Option<&str>) -> PathBuf {
    if let Some(output) = output_override {
        let candidate = PathBuf::from(output);
        if candidate.is_absolute() {
            candidate
        } else {
            project_root.join(candidate)
        }
    } else {
        project_root.join("build").join("registerspeiling.json")
    }
}

fn upsert_trusted_registry_sha256(
    config_path: &std::path::Path,
    digest: &str,
) -> std::io::Result<bool> {
    if let Some(parent) = config_path.parent() {
        fs::create_dir_all(parent)?;
    }
    let raw = match fs::read_to_string(config_path) {
        Ok(raw) => raw,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => String::new(),
        Err(err) => return Err(err),
    };
    let desired_line = format!("trusted_registry_sha256 = \"{digest}\"");
    let mut lines: Vec<String> = raw.lines().map(|line| line.to_string()).collect();
    let had_trailing_newline = raw.ends_with('\n');

    let mut security_header_idx: Option<usize> = None;
    let mut next_section_idx: Option<usize> = None;
    for (idx, line) in lines.iter().enumerate() {
        let trimmed = line.trim();
        if trimmed == "[security]" {
            security_header_idx = Some(idx);
            continue;
        }
        if security_header_idx.is_some()
            && trimmed.starts_with('[')
            && trimmed.ends_with(']')
        {
            next_section_idx = Some(idx);
            break;
        }
    }

    let changed = if let Some(header_idx) = security_header_idx {
        let end_idx = next_section_idx.unwrap_or(lines.len());
        let mut replaced = false;
        for idx in (header_idx + 1)..end_idx {
            let trimmed = lines[idx].trim();
            if trimmed.starts_with("trusted_registry_sha256") {
                if trimmed == desired_line {
                    return Ok(false);
                }
                lines[idx] = desired_line.clone();
                replaced = true;
                break;
            }
        }
        if !replaced {
            lines.insert(end_idx, desired_line.clone());
        }
        true
    } else {
        if !lines.is_empty() && !lines.last().map(|line| line.trim().is_empty()).unwrap_or(false) {
            lines.push(String::new());
        }
        lines.push("[security]".to_string());
        lines.push(desired_line);
        true
    };

    let mut rendered = lines.join("\n");
    if had_trailing_newline || changed {
        rendered.push('\n');
    }
    fs::write(config_path, rendered)?;
    Ok(changed)
}
