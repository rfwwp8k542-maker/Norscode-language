...
- Lexing, parsing, semantic analysis, bytecode build, and binary validation are all allowed to fail fast.
...
- `run` is binary-first.
- `check` combines semantic validation and binary validation.
- `build` is bytecode-first.
- `test` is binary-first.
