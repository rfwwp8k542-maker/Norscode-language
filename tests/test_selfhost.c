...
char * selfhost__compiler__uttrykk_til_ops_og_verdier_med_miljo(nl_list_text* tokens, nl_list_text* navn, nl_list_int* miljo_verdier, nl_list_text* ops, nl_list_int* verdier) {
    ...
    while (i < nl_list_text_len(tokens)) {
        ...
        else if ((((i + 1) < nl_list_text_len(tokens)) && (nl_streq(tok_raw, "rest") || nl_streq(tok_raw, "resten"))) && nl_streq(n1, "av")) {
            tok = "%";
            tok_step = 2;
        }
        else if ((((((nl_streq(tok_raw, "mod") || nl_streq(tok_raw, "modulo")) || nl_streq(tok_raw, "modul")) || nl_streq(tok_raw, "modulus")) || nl_streq(tok_raw, "rest")) || nl_streq(tok_raw, "rest_av")) || nl_streq(tok_raw, "resten_av")) {
            tok = "%";
            tok_step = 1;
        }
