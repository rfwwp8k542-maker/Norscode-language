    ...
    def emit_expr(self, node: Any, code: list[list[Any]]):
        ...
        if isinstance(node, BinOpNode):
            ...
            if node.op.typ == "NAND":
                self.emit_expr(node.left, code)
                self.emit_expr(node.right, code)
                code.append(["BINARY_AND"])
                code.append(["UNARY_NOT"])
                return
            if node.op.typ == "NOR":
                self.emit_expr(node.left, code)
                self.emit_expr(node.right, code)
                code.append(["BINARY_OR"])
                code.append(["UNARY_NOT"])
                return
            if node.op.typ in {"EKVIVALENT"}:
                self.emit_expr(node.left, code)
                self.emit_expr(node.right, code)
                code.append(["COMPARE_EQ"])
                return
            if node.op.typ == "IMPLISERER":
                false_label = self.new_label("impl_false")
                end_label = self.new_label("impl_end")
                self.emit_expr(node.left, code)
                code.append(["JUMP_IF_FALSE", false_label])
                self.emit_expr(node.right, code)
                code.append(["JUMP", end_label])
